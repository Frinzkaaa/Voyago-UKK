...
<div id="ticketList" class="flex flex-col gap-4">
  @forelse($tickets as $ticket)
    ...
    <div
      class="bg-white border-2 border-gray-100 rounded-[2rem] p-6 hover:border-orange-300 transition-all flex flex-col gap-4 group">
      ...
      <div class="flex items-center justify-between">
        @if(isset($ticket->departure_time))
          <div class="flex items-center gap-12 flex-grow">
            <div class="flex flex-col">
              <span class="text-gray-800 font-black text-[2rem] leading-none">{{ $departureTime }}</span>
            </div>
            ...
            <div class="flex flex-col">
              <span class="text-gray-800 font-black text-[2rem] leading-none">{{ $arrivalTime }}</span>
            </div>
          </div>
        @else
          ...
          <div class="flex items-center gap-12 flex-grow">
            <div class="flex flex-col">
              <span class="text-gray-600 font-bold text-sm"><i class="fas fa-map-marker-alt text-orange-500 mr-2"></i>
      ...
      <div class="flex gap-4 border-t border-gray-50 pt-3">
        <button
          class="text-xs font-black text-gray-800 hover:text-orange-500 transition-colors uppercase tracking-widest">Detail
          {{ isset($ticket->price_per_night) ? 'Hotel' : 'Perjalanan' }}</button>
        <button
          class="text-xs font-black text-gray-800 hover:text-orange-500 transition-colors uppercase tracking-widest">Info</button>
      </div>
    </div>
  @empty
