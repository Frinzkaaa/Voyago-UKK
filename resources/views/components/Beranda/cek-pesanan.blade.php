...
<div class="flex flex-col gap-6">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    @guest
      <div
        class="col-span-1 md:col-span-3 bg-white/50 border border-dashed border-gray-200 rounded-[32px] p-12 text-center">
        ...
        <h4 class="font-black text-gray-800 mb-1">Sudah punya tiket?</h4>
      ...
      @forelse($bookings as $booking)
        ...
        <div
          ...
          <div class="flex items-center justify-between">
            <span class="font-black text-gray-800 text-sm truncate max-w-[140px]">{{ $config['title'] }}</span>
            <span
          ...
          <div class="flex flex-col gap-1 px-2">
            ...
            <div class="flex items-center justify-between text-gray-800 font-black">
              ...
              <div class="flex-grow flex flex-col items-center px-4">
                ...
                <div class="w-full h-[1px] bg-gray-200 relative mb-4">
                  <i
          ...
          <div class="grid grid-cols-2 gap-4 border-t border-{{ $config['color'] }}-200/50 pt-6">
            <div class="flex flex-col">
              ...
              <span class="text-xs font-black text-gray-800 mt-1 uppercase">{{ $config['val1'] }}</span>
            </div>
            <div class="flex flex-col items-end">
              <span class="text-[8px] font-black text-gray-400 uppercase tracking-widest">{{ $config['label3'] }}</span>
              <span class="text-xs font-black text-gray-800 mt-1 uppercase">{{ $config['val3'] }}</span>
            </div>
            <div class="flex flex-col">
              <span class="text-[8px] font-black text-gray-400 uppercase tracking-widest">{{ $config['label2'] }}</span>
              <span class="text-xs font-black text-gray-800 mt-1 uppercase">{{ $config['val2'] }}</span>
            </div>
            <div class="flex flex-col items-end">
              <span class="text-[8px] font-black text-gray-400 uppercase tracking-widest">{{ $config['label4'] }}</span>
              <span class="text-xs font-black text-gray-800 mt-1 uppercase text-right">{{ $config['val4'] }}</span>
            </div>
          </div>
          <button onclick="openTicketModal()"
            class="w-full bg-white/50 hover:bg-white text-gray-800 font-black py-3 rounded-2xl text-xs transition-all border border-{{ $config['color'] }}-200/50 shadow-sm active:scale-95">Lihat
            Detail</button>
        </div>
      @empty
        <div
          ...
          <h4 class="font-black text-gray-800 mb-1">Belum ada pesanan aktif</h4>
  ...
  <div id="ticketModal" class="fixed inset-0 z-[100] hidden overflow-y-auto">
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4"
      ...
      <div
        class="bg-white rounded-[32px] shadow-2xl w-full max-w-[380px] overflow-hidden transform transition-all relative">

        <!-- Ticket Header -->
        <div class="p-6 flex items-center justify-between">
          <img src="/images/Logo.png" class="h-7 object-contain">
          <span class="font-black text-gray-800 text-xs tracking-tighter">VOYAGO E-TICKET</span>
        </div>

        <div class="px-6 pb-6 text-center">
          <div class="py-10">
            <i class="fa-solid fa-qrcode text-6xl text-gray-300 mb-4"></i>
            <h3 class="text-xl font-black text-gray-800">Detail Tiket</h3>
