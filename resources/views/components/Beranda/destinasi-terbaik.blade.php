<div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-50 flex flex-col gap-6 h-full">
    ...
    <div class="relative h-[250px] rounded-3xl overflow-hidden group shadow-md">
        ...
        <!-- Float Map Card -->
        <div
            class="absolute bottom-4 left-4 bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl p-2 flex gap-3 items-center shadow-xl">
            <div class="w-16 h-12 bg-gray-200 rounded-lg overflow-hidden border border-white/20">
    ...
    <div class="flex items-center justify-between">
        <h3 class="font-black text-xl text-gray-800">Destinasi Terbaik!</h3>
        <a href="#"
    ...
    <p class="text-sm text-gray-500 -mt-4 font-medium">Rayakan hari hari bertamu dengan destinasi terbaik di Voyago</p>
    ...
    <div class="flex flex-col gap-4 mt-2">
        ...
        @foreach($destinations as $d)
            <div class="flex items-center gap-4 bg-white hover:bg-orange-50 p-2 rounded-2xl transition-all group">
                <img src="{{ $d['img'] }}" alt="{{ $d['name'] }}" class="w-16 h-16 rounded-2xl object-cover shadow-sm">
                <div class="flex-1 flex flex-col">
                    <span class="font-black text-gray-800 text-base leading-tight">{{ $d['name'] }}</span>
                    <span class="text-xs font-thin text-gray-400 mt-0.5">{{ $d['loc'] }}</span>
                </div>
                <div class="flex items-center gap-3">
                    <button
                        class="w-10 h-10 rounded-full border-2 border-orange-400 flex items-center justify-center text-orange-500 hover:bg-orange-500 hover:text-white transition-all">
