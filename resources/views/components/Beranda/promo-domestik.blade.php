<div class="flex flex-col gap-6">
  <div class="flex flex-col gap-4">
    <h3 class="font-black text-2xl text-gray-800">Promo Tiket Pesawat Domestik Murah!</h3>
    ...
    <div class="flex items-center justify-between">
      <div class="flex flex-wrap gap-2">
        ...
        @foreach($tabs as $t)
          <button
            class="{{ $loop->first ? 'bg-orange-500 text-white shadow-md shadow-orange-100' : 'bg-white text-gray-500 hover:text-orange-500 border border-gray-100' }} px-4 py-1.5 rounded-full text-xs font-black transition-all">
            {{ $t }}
          </button>
        @endforeach
      </div>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
    ...
    @foreach($promos as $p)
      <div
        class="bg-white rounded-3xl p-4 shadow-sm border border-gray-50 group cursor-pointer hover:border-orange-200 transition-all flex flex-col gap-4">
        ...
        <div class="flex flex-col px-1">
          <h4 class="font-black text-gray-800 text-sm leading-tight group-hover:text-orange-500 transition-colors">Jakarta
            - Bali / Denpasar</h4>
