<div class="flex flex-col gap-6 mb-12">
  ...
  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 md:gap-6">
    ...
    @foreach($guides as $g)
      <div
        class="group relative h-56 md:h-72 rounded-2xl md:rounded-3xl overflow-hidden cursor-pointer shadow-md transform hover:scale-[1.05] transition-all">
        <img src="{{ $g['img'] }}"
          class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div>
        <div class="absolute bottom-6 left-1/2 -translate-x-1/2 w-full text-center px-4">
          <h4 class="text-white font-black text-base md:text-lg leading-tight group-hover:text-orange-400 transition-colors">
            {{ $g['city'] }}
          </h4>
