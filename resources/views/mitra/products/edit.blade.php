...
@section('content')
    <div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="space-y-1">
            <h2 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight uppercase italic">
                Edit @if ($type == 'hotel') Akomodasi @elseif($type == 'pesawat') Flight Listing @elseif($type == 'kereta') Rail Management @elseif($type == 'bus') Transport Entry @else Atraksi Wisata @endif
            </h2>
            <p class="text-sm text-gray-500 dark:text-zinc-400 font-medium">Perbarui informasi layanan Anda untuk menjaga data tetap akurat.</p>
        </div>
    ...
    <form action="{{ route('partner.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="pb-20">
        @csrf
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div class="lg:col-span-8 space-y-10">
                ...
                <div class="bg-white dark:bg-zinc-900 p-10 rounded-[40px] border border-gray-100 dark:border-zinc-800 shadow-sm">
                    ...
                    @include('mitra.products.forms.' . ($type == 'pesawat' ? 'flight' : ($type == 'kereta' ? 'train' : ($type == 'bus' ? 'bus' : ($type == 'hotel' ? 'hotel' : 'wisata')))), ['product' => $product])
                ...
                <div class="bg-white dark:bg-zinc-900 p-10 rounded-[40px] border border-gray-100 dark:border-zinc-800 shadow-sm relative overflow-hidden group">
                    ...
                    <textarea name="terms" rows="4"
                        class="w-full bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 focus:border-orange-500 rounded-[32px] py-6 px-8 outline-none transition-all text-sm font-bold text-gray-900 dark:text-white leading-relaxed resize-none">{{ $product->terms ?? '' }}</textarea>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 space-y-10">
                    ...
                    <div class="space-y-8 relative z-10">
                        <div>
                            ...
                            <div class="relative group">
                                ...
                                <input type="number" name="price" required 
                                    value="{{ (int)($product->price ?? $product->price_per_night) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="stock" required 
                                value="{{ $product->seats_available ?? $product->availability }}"
                ...
                <div class="bg-white dark:bg-zinc-900 p-10 rounded-[40px] border border-gray-100 dark:border-zinc-800 shadow-sm">
                    ...
                    <div class="space-y-10">
                        <x-Form.image-uploader name="image" label="Foto Cover Profil" helper="Rasio 16:9 disarankan" :value="$product->image" />

                        @if($type == 'hotel')
                                <x-Form.image-uploader name="front_image" label="Fasad Bangunan" :value="$product->front_image" />
                                <x-Form.image-uploader name="room_images" label="Interior Kamar" :multiple="true" :value="$product->room_images" />
                        @elseif($type == 'wisata' || $type == 'bus')
                            <x-Form.image-uploader name="main_image" label="Foto Ikonik" :value="$product->main_image" />
                        @endif

                        <div class="pt-8 border-t border-gray-50 dark:border-zinc-800">
                             <x-Form.image-uploader name="gallery" label="Media Tambahan" :multiple="true" :value="$product->gallery" />
                        </div>
                    </div>
                </div>
                @endif
